...
GRADE_LETTER_TO_NUM_SCALE = {
    'A+' : [12, 4.0], 'A' : [11, 3.9], 'A-' : [10, 3.7],
    'B+' : [9, 3.3], 'B' : [8, 3.0], 'B-' : [7, 2.7],
    'C+' : [6, 2.3], 'C' : [5, 2.0], 'C-' : [4, 1.7],
    'D+' : [3, 1.3], 'D' : [2, 1.0], 'D-' : [1, 0.7],
    'F' : [0, 0],
}
...
def calculate_avg(grade_data: list) -> tuple:
    ...
    for (_, units, grades) in grade_data:
        ...
        if grades.text in GRADE_LETTER_TO_NUM_SCALE:
            # 1st index is for 12pt scale, second is for 4pt scale
            grade_total[0] += GRADE_LETTER_TO_NUM_SCALE[grades.text][0] * float(units.text)
            grade_total[1] += GRADE_LETTER_TO_NUM_SCALE[grades.text][1] * float(units.text)
